...
Ts=0.01;
n=4000;
u=[0.03*ones(1,n) 0.06*ones(1,n) 0.09*ones(1,n)];
t=0:Ts:size(u,2)*Ts-Ts;
x = trapz(t,u/0.0154)

SetPump_vector = timeseries(u,t);

%% processing
load('4229_results.mat')
nocompdata=mdfData;
load('4232_results.mat')
compdata=mdfData;
figure(1)
hold on
plot(seconds(nocompdata.Time),double(nocompdata.ModelRoot_controller_controller_ModelRoot_ScopeHeightL_mm__In1))
plot(seconds(nocompdata.Time),double(nocompdata.ModelRoot_controller_controller_ModelRoot_ScopeHeightR_mm__In1))
plot(seconds(compdata.Time),double(compdata.ModelRoot_controller_controller_ModelRoot_ScopeHeightL_mm__In1))
plot(seconds(compdata.Time),double(compdata.ModelRoot_controller_controller_ModelRoot_ScopeHeightR_mm__In1))
plot(t,cumsum(u)*0.01/0.0154);
legend('x_1 no compensation','x_2 no compensation','x_1 compensation','x_2 compensation','reference')
grid on
